...
import { 
  ...
  Download, BarChart3, ChevronLeft, TrendingUp, Send, Box, Copy
} from 'lucide-react';
import { 
...
export default function ParcelTracker() {
  ...
  const [products, setProducts] = useState([]); // 物品记忆库
  ...
  // --- 1. 初始化 ---
  useEffect(() => {
    const storedId = localStorage.getItem('parcel_household_id'); // 改个key名字避免混淆
    if (storedId) {
    ...
    if (isSupabaseReady && !isOffline) {
      try {
        ...
        const formattedItems = pkgData.map(d => ({
          ...
          trackingNumber: d.tracking_number, // 单号
          status: d.status,
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `快递记录_${new Date().toLocaleDateString()}.csv`;
  ...
  const itemStats = useMemo(() => {
    return products.map(prod => {
      ...
      const avgPrice = totalQty > 0 ? relatedItems.reduce((sum, i) => sum + (i.costPrice * i.quantity), 0) / totalQty : 0;
      return { ...prod, realTotalQty: totalQty, avgPrice: avgPrice.toFixed(2), recordCount: relatedItems.length };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 font-sans text-gray-800">
      {toast && (
        <div className="fixed top-24 left-1/2 transform -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-2 duration-300 bg-gray-800 text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg">
          {toast}
        </div>
      )}
          ...
          <div className="space-y-4">
            {selectedStatId ? (
                ...
                <div className="h-64 w-full mb-6 bg-gray-50 rounded-xl p-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                      <defs><linearGradient id="colorPrice" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={0.3}/><stop offset="95%" stopColor="#2563eb" stopOpacity={0}/></linearGradient></defs>
              ...
              <div className="grid gap-3">
                ...
                {itemStats.map(prod => (
                  <div key={prod.id} onClick={() => setSelectedStatId(prod.id)} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex justify-between items-center cursor-pointer hover:border-blue-300 transition-all">
                    <div>
                      <div className="font-bold text-gray-800">{prod.name}</div>
                      <div className="text-xs text-gray-500 mt-1">累计收到 {prod.totalQty} 件</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-blue-600">价值 ¥{prod.last_price}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <input type="text" className="w-full p-4 bg-gray-50 rounded-xl border-transparent focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none font-mono text-lg" 
                  placeholder="点击扫描或粘贴" value={newItem.trackingNumber} onChange={e => setNewItem({...newItem, trackingNumber: e.target.value})} />
              </div>
              ...
              <div className="relative">
                ...
                <input required type="text" className="w-full p-4 bg-gray-50 rounded-xl border-transparent focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none" 
                  placeholder="例如：衣服、文件..." value={newItem.productName} onChange={e => handleNameChange(e.target.value)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)} />
                {/* 联想列表 */}
                  ...
                  <div className="absolute z-20 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                    {suggestions.map(s => (
                      <div key={s.id} onClick={() => selectSuggestion(s)} className="p-3 hover:bg-blue-50 cursor-pointer border-b border-gray-50 flex justify-between">
                        ...
                        <span className="text-xs text-gray-400">价值: ¥{s.last_price}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">收件人</label><input type="text" className="w-full p-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" placeholder="谁的快递?" value={newItem.recipient} onChange={e => setNewItem({...newItem, recipient: e.target.value})} /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">发件人/店铺</label><input type="text" className="w-full p-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" value={newItem.supplier} onChange={e => setNewItem({...newItem, supplier: e.target.value})} /></div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">物品价值 (元)</label><input type="number" className="w-full p-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" placeholder="选填" value={newItem.costPrice} onChange={e => setNewItem({...newItem, costPrice: e.target.value})} /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">包裹数量</label><input type="number" className="w-full p-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" value={newItem.quantity} onChange={e => setNewItem({...newItem, quantity: e.target.value})} /></div>
              </div>
          ...
          <>
             {view === 'dashboard' && (
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-orange-50 p-5 rounded-2xl border border-orange-100 flex flex-col items-center justify-center cursor-pointer active:scale-95 transition-transform" onClick={() => setView('list')}>
                    <span className="text-orange-400 text-xs font-bold uppercase tracking-wider mb-1">待收包裹</span>
                    <span className="text-3xl font-extrabold text-orange-600">{items.filter(i => i.status !== 'received').length}</span>
                  </div>
                  <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 flex flex-col items-center justify-center" onClick={() => setView('stats')}>
                    <span className="text-blue-400 text-xs font-bold uppercase tracking-wider mb-1">累计货值</span>
                    <span className="text-3xl font-extrabold text-blue-600">¥{items.reduce((a,b)=>a+(b.costPrice*b.quantity),0).toLocaleString()}</span>
                  </div>
                </div>
             )}
             ...
             {view === 'dashboard' && (
               <div className="flex justify-end mb-2">
                  <button onClick={handleExport} className="text-xs flex items-center gap-1 bg-white text-gray-600 px-3 py-1.5 rounded-lg border border-gray-200 font-medium hover:bg-gray-50">
                    <Download className="w-3 h-3" /> 导出Excel
                  </button>
               </div>
             )}
                 ...
                 <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 text-sm space-y-1.5 border border-gray-100 text-gray-600 mb-3">
                      <div className="flex justify-between items-center">
                        {item.recipient ? <span className="flex items-center gap-1 font-bold"><User className="w-3 h-3"/> {item.recipient}</span> : <span></span>}
                        {item.costPrice > 0 && <span className="text-xs bg-white border px-1 rounded">¥{item.costPrice}</span>}
                      </div>
                      {item.supplier && <div className="flex items-center gap-1 text-xs text-gray-500"><Send className="w-3 h-3"/> 发: {item.supplier}</div>}
                      {item.trackingNumber && (
                        <div onClick={() => copyToClipboard(item.trackingNumber)} className="font-mono text-xs text-gray-500 bg-white p-2 rounded border border-gray-200 truncate mt-1 flex justify-between items-center cursor-pointer hover:bg-gray-50 active:bg-gray-100">
                    ...
                    <div className="flex gap-2">
                      {item.status !== 'received' ? (
                        <button onClick={() => handleStatusUpdate(item.id, 'received')} className="flex-1 bg-blue-600 text-white py-2 rounded-lg text-sm font-bold hover:bg-blue-700 shadow-md shadow-blue-200">确认签收</button>
                      ) : (
                        <button className="flex-1 bg-gray-100 text-gray-400 py-2 rounded-lg text-sm font-bold cursor-default">已入库</button>
                      )}
